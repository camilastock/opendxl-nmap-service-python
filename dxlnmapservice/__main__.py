...
if len(sys.argv) != 2:
    print "Usage: dxlnmapservice <configuration files directory>"
    sys.exit(1)
...
logging_config_path = os.path.join(config_dir, NmapService.LOGGING_CONFIG_FILE)
...
with NmapService(sys.argv[1]) as app:
    try:
        # Run the application
        app.run()
        running = True
